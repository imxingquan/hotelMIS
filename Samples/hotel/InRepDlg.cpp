...
// CInRepDlg 对话框
 CString CInRepDlg::m_exp;
 CString CInRepDlg::m_hotelname;

IMPLEMENT_DYNAMIC(CInRepDlg, CDialog)
CInRepDlg::CInRepDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInRepDlg::IDD, pParent)
	, m_py(_T(""))
	,m_bIsTree(TRUE)
	, m_title(_T(""))
	, m_inout_dh(_T(""))
...
void CInRepDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CInRepDlg, CDialog)
...
BOOL CInRepDlg::OnInitDialog()
{
	...
	m_GuiStaticTitle.SetWindowText(_T("商品入库"));
	
	CTime t = CTime::GetCurrentTime();
	m_inout_dh.Format("%s",t.Format("RK%y%m%d%H%M%S"));
	...
	m_exp.Format("账单号:%s   入库日期:%s",m_inout_dh,t.Format("%y-%m-%d %H:%M:%S"));
...
void CInRepDlg::Init()
{
	...
	m_GuiListRep.InsertColumn(++i,_T("数量"),LVCFMT_LEFT,50);
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnEnChangeEditPy()
{
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonExt()
{
	// 树,收缩与展开
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonTree()
{
	m_bIsTree = TRUE;
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonList()
{
	m_bIsTree = FALSE;
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonMoveRight()
{
	if ( m_isIn == true)	//已经操作完毕
		return;
	...
	if ( dlg.DoModal() == IDOK){
		...
		inRep.rep_flag = hotelMIS::HInOutRep::IN_REP;
		hotelMIS::HBehindRep bRep = behindRepCtrl.calc(food_id,num,price,hotelMIS::HInOutRep::IN_REP);
...
//----------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnNMClickMenuTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	if ( m_isIn == true)
		return;
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnNMDblclkMenuTree(NMHDR *pNMHDR, LRESULT *pResult)
{
		if ( m_isIn == true)
		return;
...
//------------------------------------------------------------------------------------------------------
void CInRepDlg::OnNMClickMenuList(NMHDR *pNMHDR, LRESULT *pResult)
{
		if ( m_isIn == true)
		return;
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnNMDblclkMenuList(NMHDR *pNMHDR, LRESULT *pResult)
{
		if ( m_isIn == true)
		return;
	// 左边 商品列表
...
//--------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonMoveLeft()
{
		if ( m_isIn == true)
		return;
...
//-------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnNMClickListInrep(NMHDR *pNMHDR, LRESULT *pResult)
{
		if ( m_isIn == true)
		return;
...
//--------------------------------------------------------------------------------------------------------------
void CInRepDlg::OnBnClickedButtonInrep()
{
		if ( m_isIn == true)
		return;

	//入库
	int r = MessageBox(_T("确实要入库!"),_T("提示"),MB_ICONINFORMATION|MB_YESNO);
	if ( r == IDNO)
		return;
	...
	MessageBox(_T("商品入库成功!"),_T("提示"),MB_ICONINFORMATION|MB_OK);
	
}

void CInRepDlg::OnBnClickedButtonPrint()
{
	/*InPrint print(m_listInOutRep);
	if ( print.CreateExcel() == true){
		print.LoadTemplate("\\tmpl\\inrep");
		print.SendExcel();
		print.SaveAs(_T("入库单"));
		print.Print();
	}*/
...
void CInRepDlg::OnBnClickedButtonPerview()
{
	//打印预览 
	/*InPrint print(m_listInOutRep);
	if ( print.CreateExcel() == true){
...
void CInRepDlg::DrawDC(CDC &memDC, PRNINFO PrnInfo)
{
if(memDC.m_hDC == NULL)
		return;
	...
	TCHAR szTitle[255];
	wsprintf(szTitle,TEXT("%s 入库单"),m_hotelname.GetBuffer());
...
HBRUSH CInRepDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
